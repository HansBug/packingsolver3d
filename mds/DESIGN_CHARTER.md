...
> **Amendment (2026-09-18, time budget).** Rule 7 keeps limits upstream's, but the caller still has to choose one. `packingsolver3d.estimate` adds `recommend_time_budget(instance, solver, alpha, speed)`: a pure function of the instance that returns a `TimeBudget` (time limit as a loose upper bound plus the stagnation-stop knobs), from a model fitted on 3158 recorded anytime curves with one interpretable formula per upstream algorithm path (`algorithm_path` replicates upstream's selection); `alpha` defaults per solver (`box` 4, `boxstacks` 8) and the time limit is always paired with the stagnation-stop knobs, because a replay on every curve showed neither a bare limit nor a bare stagnation stop is robust on its own. Constants live in the generated `_time_budget_constants.py` with the reference machine and upstream commit recorded; the campaign is reproducible from `experiments/time_budget/`. No test asserts wall-clock durations.

> **Amendment (2026-09-20, relative patience).** The stagnation stop gains `stop_when_unimproved_ratio`: the patience becomes `max(for, ratio × time of the last improvement)` and the watchdog never fires before a first solution. Upstream's anytime searches double their queues between passes, so a fixed patience either cuts late passes short or (as 0.0.3's `(T − L)/2` did) wastes half the budget; the ratio follows the pass structure. `recommend_time_budget` sets it to `alpha / 2` clamped to 1–4 with a 5 s floor and the covered latency as the earliest stop, replacing the `T/2` rule.
