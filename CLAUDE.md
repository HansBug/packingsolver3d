...
7. **Limits are upstream's.** `time_limit` goes to upstream's timer and `memory_limit` to its own memory check. Choosing the limit is the caller's job, helped by `estimate.recommend_time_budget`: a pure function of the instance (no solve, no clock) whose constants are generated into `_time_budget_constants.py` by `experiments/time_budget/`; never hand-edit them, and never add a test that asserts wall-clock durations. There is no process boundary: a crash inside upstream takes the interpreter with it, and the docstrings say so. Do not add a hidden subprocess or thread-kill layer; callers who need isolation own their worker process. The stagnation watchdog behind `stop_when_unimproved_for` is not such a layer: a side thread that never touches Python and only raises upstream's own stop signal (an end boolean on its timer), exactly what a time limit does.
