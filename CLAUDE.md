...
- `benchmarks/<topic>/` -- the capability study: `datasets`, `participants`, `protocol`, `leaderboards`, `gallery`. The tables under `benchmarks/_generated/` and the figures under `_static/benchmarks/` are written by `tools/make_benchmarks.py` (`make benchmarks`) and are never edited by hand; see the Benchmarks section below.
...
- Adding a case or participant means: extend `BENCHMARKS`/`PARTICIPANTS` in `tools/make_benchmarks.py`, add the placements to `third_party.json`, run `make benchmarks`, update the prose in both languages where it quotes numbers, and keep README's summary table in step.
