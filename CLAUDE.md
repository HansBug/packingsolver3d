...
`test/testfile/upstream/` holds verbatim copies of the instances upstream's own gtests solve (`data/box/tests/*`, `data/boxstacks/tests/*`, see `SOURCE.md` there), and `test/test_upstream_cases.py` replays each gtest through this package: same files, same parameters (`use_tree_search`, `not-anytime-sequential`), and the comparison `Solution::operator<` uses for the objective. When the submodule moves, diff upstream's `test/box` and `test/boxstacks` directories and port what changed; do not invent cases there -- hand-written scenarios belong in the other test modules.
...
- `how_to/<topic>/` -- task pages for readers who know what they want (`installation`, `budgets`, `upstream_cases`).
