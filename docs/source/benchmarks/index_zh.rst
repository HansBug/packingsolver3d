...
这一节是一项小规模、可完整复现的能力研究。它把 packingsolver3d，也就是上游 PackingSolver 的 ``box`` 求解器，放到三个公开的三维装箱实例族上，与 Python、Go、Rust 生态里最常被拿来用的开源三维装箱库以及两个作为参照的精确程序一起跑。这些实例族小到可以画出来，97 个 case 中大多数的最优值已知或已证明，所以表格能把"找到了一个不错的装法"和"找到了最优装法"区分开，并指出十秒对谁都不够用的地方。
...
* :doc:`leaderboards/index_zh` -- 完整结果：参与者名录、跨基准汇总，以及每个基准的 case 与界、天梯（一行一个参与者，一列一个 case）、装入件数和用时。
...
solve 一步以 10 s 时限对 97 个 case 调用 ``box.solve`` 并重写 ``tools/benchmarks/ours.json``\ ；render 一步重新校验每个存下来的解（我们的和第三方的一样），从摆放位置重算目标值，并重新生成本节的表格与图。
